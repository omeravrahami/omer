...
export function InsightsCards({
  ...
  const cards = useMemo<InsightCard[]>(() => {
    ...
    // 2. שכר לשעה נטו
    if (hoursWorkedThisMonth > 0 && net > 0) {
      const netHourly = net / hoursWorkedThisMonth;
      result.push({
        id: 'hourly-net',
        title: 'שכר לשעה נטו',
        mainValue: formatCurrency(netHourly),
        subtitle: `לאחר מסים · ${hoursWorkedThisMonth.toFixed(1)} שע׳ החודש`,
        accentColor: '#2563EB',
        bgColor: '#EFF6FF',
        borderColor: 'rgba(37,99,235,0.18)',
        icon: <Clock size={14} color="#2563EB" strokeWidth={2.2} />,
      });
    }
